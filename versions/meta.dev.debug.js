...
meta.decodeBinaryBase64 = function(content) {
    ...
    for (var n = 0, i = 0; n < size; n += 4, i++) {
        data[i] = decodedData.charCodeAt(n) | decodedData.charCodeAt(n + 1) << 8 | decodedData.charCodeAt(n + 2) << 16 | decodedData.charCodeAt(n + 3) << 24;
    }
    return data;
};
...
meta.class("meta.Controller", {
    init: function() {
        this.view = meta.createView("__ctrl__" + this.__lastName__ + meta.cache.ctrlUniqueID++);
        if (this.onInit) {
            this.onInit();
        }
    },
...
meta.class("Resource.Texture", "Resource.Basic", {
    ...
    createAnim: function(frameNames) {
        this._numToLoad = 0;
        var texture, textureName, wildCard, wildCardIndex;
        var textures = meta.resources.textures;
    ...
    resizeSilently: function(width, height) {
        ...
        this.width = this.frameWidth * unitRatio + .5 | 0;
        this.height = this.frameHeight * unitRatio + .5 | 0;
    ...
    draw: function(ctx, x, y) {
        if (!this.ptr) {
            ...
            ctx.drawImage(this.ptr.canvas, this.x, this.y, this.width, this.height, x, y, this.width, this.height);
        }
    },
    drawFrame: function(ctx, x, y, frame) {
        if (this.frameData) {
            ...
            ctx.drawImage(this.canvas, this.frameWidth * (frame % this.framesX), this.frameHeight * Math.floor(frame / this.framesX), this.frameWidth, this.frameHeight, x, y, this.frameWidth, this.frameHeight);
        }
    },
    clear: function() {
        ...
        if (rawFramesX !== framesX || rawFramesY !== framesY) {
            ...
            for (var y = 0; y < framesY; y++) {
                ...
                for (var x = 0; x < framesX; x++) {
                    ...
                    this.frames[id] = this._createTexture(this.texture.name + id, posX, posY, frameWidth + offsetX, frameHeight + offsetY);
                    id++;
                }
            }
        } else {
    ...
    loadXML: function(xml) {
        ...
        for (var i = 0; i < numNodes; i++) {
            ...
            if (node.nodeName === "SubTexture") {
                this._createTexture(node.getAttribute("name"), node.getAttribute("x"), node.getAttribute("y"), node.getAttribute("width"), node.getAttribute("height"));
            } else if (node.nodeName === "sprite") {
                this._createTexture(node.getAttribute("n"), node.getAttribute("x"), node.getAttribute("y"), node.getAttribute("w"), node.getAttribute("h"));
        ...
        for (var i = 0; i < numNodes; i++) {
            ...
            if (node.nodeName === "key") {
                ...
                if (command === "frame") {
                    ...
                    this._createTexture(name, parseInt(data[0]), parseInt(data[1]), parseInt(data[2]), parseInt(data[3]));
                    return;
                }
            }
        }
...
meta.class("Entity.Geometry", {
    ...
    set picking(value) {
        if (value) {
            ...
            if ((this.flags & this.Flag.PICKING) === 0) {
                return;
            }
            this.flags &= ~this.Flag.PICKING;
            if (this.flags & this.Flag.RENDER) {
...
meta.class("Entity.Tilemap", "Entity.Geometry", {
    ...
    _parse_tmx: function(data) {
        ...
        this.create(parseInt(node.getAttribute("width")), parseInt(node.getAttribute("height")), parseInt(node.getAttribute("tilewidth")), parseInt(node.getAttribute("tileheight")), node.getAttribute("orientation"));
...
meta.class("Entity.TilemapLayer", "Entity.Geometry", {
    ...
    updateFromData: function() {
        this.resize(this.tilesX * this.tileWidth + this.tileOffsetX, this.tilesY * this.tileHeight + this.tileOffsetY);
        if (!this._dataInfo) {
...
meta.class("Entity.TilemapOrthoLayer", "Entity.TilemapLayer", {
    ...
    draw: function(ctx) {
        if ((this.parent.flags & this.Flag.LOADED) === 0) {
            return;
        }
        var cameraVolume = meta.camera.volume;
        ...
        if (this._dataFlags) {
                ...
                for (var x = startTileX; x < endTileX; x++) {
                    ...
                    if (info) {
                        ...
                        if (flags) {
                            ...
                            ctx.drawImage(info.canvas, info.posX, info.posY, info.width, info.height, posX, posY, info.width, info.height);
                        }
                    }
                    id++;
                    posX += this.tileWidth;
            ...
            for (var y = startTileY; y < endTileY; y++) {
                ...
                for (var x = startTileX; x < endTileX; x++) {
                    texture = this._dataInfo[id];
                    if (texture) {
                        ctx.drawImage(texture.canvas, texture.x, texture.y, texture.width, texture.height, posX, posY - texture.height + this.tileHeight, texture.width, texture.height);
                    }
                    if (this._cells) {
    ...
    getPos: function(cellX, cellY) {
        var x = this.tilesX * this.tileHalfWidth - cellY * this.tileHalfWidth - this.tileHalfWidth + cellX * this.tileHalfWidth;
        var y = y = cellY * this.tileHalfHeight + this.tileOffsetY + cellX * this.tileHalfHeight;
        return [ x, y ];
    },
    getWorldPos: function(cellX, cellY) {
        var x = this.tilesX * this.tileHalfWidth - cellY * this.tileHalfWidth - this.tileHalfWidth + cellX * this.tileHalfWidth + this.volume.minX;
        var y = y = cellY * this.tileHalfHeight + this.tileOffsetY + cellX * this.tileHalfHeight + this.volume.minY;
        return [ x, y ];
    },
    getPosEx: function(cellPos) {
        cellPos.x = this.tilesX * this.tileHalfWidth - cellPos.cellY * this.tileHalfWidth - this.tileHalfWidth + cellPos.cellX * this.tileHalfWidth;
        cellPos.y = cellPos.cellY * this.tileHalfHeight + this.tileOffsetY + cellPos.cellX * this.tileHalfHeight;
    },
    getWorldPosEx: function(cellPos) {
        cellPos.x = this.tilesX * this.tileHalfWidth - cellPos.cellY * this.tileHalfWidth - this.tileHalfWidth + cellPos.cellX * this.tileHalfWidth + this.volume.minX;
        cellPos.y = cellPos.cellY * this.tileHalfHeight + this.tileOffsetY + cellPos.cellX * this.tileHalfHeight + this.volume.minY;
    },
    ...
    getPosY: function(cellX, cellY) {
        var y = cellY * this.tileHalfHeight + this.tileOffsetY + cellX * this.tileHalfHeight;
        return y + 20;
    }
});
...
meta.class("Entity.TilemapIsoLayer", "Entity.TilemapLayer", {
        ...
        if (this._dataFlags) {
            for (var y = startTileY; y < this.tilesY; y++) {
                ...
                for (var x = startTileX; x < endTileX; x++) {
                    texture = this._dataInfo[id];
                    if (texture) {
                        ctx.drawImage(texture.canvas, texture.x, texture.y, texture.width, texture.height, posX, posY + this.tileHeight - texture.height, texture.width, texture.height);
                    }
                    id++;
    ...
    _calcEntityPos: function(entity) {
        var x = this.tilesX * this.tileHalfWidth - entity.cellY * this.tileHalfWidth - this.tileHalfWidth + entity.cellX * this.tileHalfWidth;
        var y = entity.cellY * this.tileHalfHeight + this.tileOffsetY + entity.cellX * this.tileHalfHeight;
        entity._x = x + this.tileHalfWidth;
        entity._y = y + this.tileHalfHeight;
        entity.updatePos();
    },
    getPos: function(cellX, cellY) {
        var x = this.tilesX * this.tileHalfWidth - cellY * this.tileHalfWidth + cellX * this.tileHalfWidth;
        var y = cellY * this.tileHalfHeight + cellX * this.tileHalfHeight + this.tileHalfHeight + this.tileOffsetY;
        return [ x, y ];
    },
    getWorldPos: function(cellX, cellY) {
        var x = this.tilesX * this.tileHalfWidth - cellY * this.tileHalfWidth + cellX * this.tileHalfWidth + this.volume.minX;
        var y = cellY * this.tileHalfHeight + cellX * this.tileHalfHeight + this.tileHalfHeight + this.tileOffsetY + this.volume.minY;
        return [ x, y ];
    },
    getPosEx: function(cellPos) {
        cellPos.x = this.tilesX * this.tileHalfWidth - cellPos.cellY * this.tileHalfWidth + cellPos.cellX * this.tileHalfWidth;
        cellPos.y = cellPos.cellY * this.tileHalfHeight + cellPos.cellX * this.tileHalfHeight + this.tileHalfHeight + this.tileOffsetY;
    },
    getWorldPosEx: function(cellPos) {
        cellPos.x = this.tilesX * this.tileHalfWidth - cellPos.cellY * this.tileHalfWidth + cellPos.cellX * this.tileHalfWidth + this.volume.minX;
        cellPos.y = cellPos.cellY * this.tileHalfHeight + cellPos.cellX * this.tileHalfHeight + this.tileHalfHeight + this.tileOffsetY + this.volume.minY;
    },
    getPosX: function(cellX, cellY) {
        var x = this.tilesX * this.tileHalfWidth - cellY * this.tileHalfWidth + cellX * this.tileHalfWidth;
        return x;
    ...
    getPosY: function(cellX, cellY) {
        var y = cellY * this.tileHalfHeight + cellX * this.tileHalfHeight + this.tileOffsetY + this.tileOffsetY;
        return y;
    },
...
meta.class("Entity.TilemapHexLayer", "Entity.TilemapLayer", {
    draw: function(ctx) {
        ...
        if (this.layerFlags & this.LayerFlag.CONVERTED_TO_ENTITIES) {
            return;
        }
        var cameraVolume = meta.camera.volume;
        var startTileX = 0;
        ...
        if (this._dataFlags) {
            ...
            for (var y = startTileY; y < this.tilesY; y++) {
                for (var x = startTileX; x < this.tilesX; x++) {
                    texture = this._dataInfo[id++];
                    if (texture) {
                        ctx.drawImage(texture.canvas, texture.x, texture.y, texture.width, texture.height, posX, posY, texture.width, texture.height);
                    }
                    posX += this.tileWidth;
                }
                flipX ^= 1;
    ...
    getPosEx: function(cellPos) {
        cellPos.x = this.tilesX * this.tileHalfWidth - cellPos.cellY * this.tileHalfWidth - this.tileHalfWidth + cellPos.cellX * this.tileHalfWidth;
        cellPos.y = y = cellPos.cellY * this.tileHalfHeight + this.tileOffsetY + cellPos.cellX * this.tileHalfHeight;
    },
    getWorldPosEx: function(cellPos) {
        cellPos.x = this.tilesX * this.tileHalfWidth - cellPos.cellY * this.tileHalfWidth - this.tileHalfWidth + cellPos.cellX * this.tileHalfWidth + this.volume.minX;
        cellPos.y = y = cellPos.cellY * this.tileHalfHeight + this.tileOffsetY + cellPos.cellX * this.tileHalfHeight + this.volume.minY;
    },
    getPosX: function(cellX, cellY) {
        var x = this.tilesX * this.tileHalfWidth - cellY * this.tileHalfWidth - this.tileHalfWidth + cellX * this.tileHalfWidth;
        return x;
...
meta.class("meta.CanvasRenderer", "meta.Renderer", {
    ...
    renderEntities: function(entities) {
        ...
        for (var n = 0; n < num; n++) {
            entity = entities[n];
            if (entity.flags & this.entityFlag.INSTANCE_HIDDEN) {
                continue;
            }
            if (entity.draw) {
    ...
    renderDebugVolumes: function() {
        if (meta.engine.flags & meta.engine.Flag.LOADING) {
            return;
        }
        this.ctx.save();
        this.setProjection(this.perspectiveProjection);
...
Steering.Component.prototype = {
    ...
    seek: function() {
        var volume = this.owner.volume;
        var desired = new meta.math.Vector2(this.target.x - volume.x, this.target.y - volume.y);
        desired.normalize();
        desired.mulValue(this.maxSpeed);
        var steer = new meta.math.Vector2(desired.x - this.vel.x, desired.y - this.vel.y);
