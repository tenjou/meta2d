...
class Input
{
	...
	handleMouseEvent(eventType, domEvent)
	{
		...
		this.prevX = this.x
		this.prevY = this.y
		this.x = (this.screenX - transform.m[6]) / camera._scale.x / wnd.ratio | 0
		this.y = (this.screenY - transform.m[7]) / camera._scale.y / wnd.ratio | 0
	...
	handleTouchEvent(domEvent, eventType)
	{
		...
		for(let n = 0; n < changedTouches.length; n++)
		{
			...
			const x = (screenX - transform.m[6]) / camera._scale.x | 0
			const y = (screenY - transform.m[7]) / camera._scale.y | 0
