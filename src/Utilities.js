...
meta.isSpace = function(c) {
	return (c === " " || c === "\t" || c === "\r" || c === "\n");
};

meta.isNewline = function(c) {
	return (c === "\r" || c === "\n");
};

meta.isDigit = function(c) {
	return (c >= "0" && c <= "9") || (c === ".");
};

meta.isAlpha = function(c) 
{
	return (c >= "a" && c <= "z") ||
		   (c >= "A" && c <= "Z") ||
		   (c == "_" && c <= "$");
...
meta.getClsFromPath = function(path)
{
	var cls = null;
	var scope = window;
	var num = path.length;
